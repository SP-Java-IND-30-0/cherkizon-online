# Cherkizon-Online — современная платформа для торговли между пользователями

**Cherkizon-Online** — это удобная и безопасная площадка для продажи, обмена и покупки товаров между пользователями. Платформа сочетает в себе высокую производительность backend-системы и интуитивно понятный интерфейс для комфортного взаимодействия.
...
- **Основные фреймворки**:
   - Spring Boot 3.4.4
   - Spring Cloud 2024.0.1 (для микросервисной архитектуры)
- **Безопасность**:
...
- **Интеграции**:
   ...
   - Yandex Object Storage (хранение медиафайлов)
   - Yandex Mail (рассылка уведомлений)

### Frontend
- **React** (динамический интерфейс)
- **Docker** (готовый образ для быстрого развертывания)

Система построена по микросервисной архитектуре с четким разделением ответственности между компонентами, что обеспечивает масштабируемость и отказоустойчивость.
...
3. При проблемах с Yandex Cloud:
